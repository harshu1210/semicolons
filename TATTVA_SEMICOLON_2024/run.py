...
from tattva.parse_dataframe import parse_dataframe
from tattva.layout import get_app_layout, get_distinct_colors, create_color_legend, DEFAULT_COLOR, DEFAULT_NODE_SIZE, DEFAULT_EDGE_SIZE
 
# class
class DashPoweredNetworkGraph:
    ...
    def _callback_search_graph(self, graph_data, search_text):
        ...
        adjusent_nodes = []
        for each in graph_data['edges']:
            if search_text.lower() in each['id'].lower():
                adjusent_nodes.extend(map(str.lower, each['id'].split('_')))
        for node in nodes:
            if search_text.lower() not in node['label'].lower() and node['label'].lower() not in adjusent_nodes:
                node['hidden'] = True
            else:
                node['hidden'] = False
    ...
    def _callback_color_nodes(self, graph_data, color_nodes_value):
        ...
        if color_nodes_value == 'None':
            ...
            # if color_nodes_value.lower() == 'cluster':
            clusters = []
            import random
            # r = lambda: random.randint(0,255)
            from layout import KELLY_COLORS_HEX
            for row in self.data['edges']:
                temp = [row['from']]
                for each_v in self.data['edges']:
                    if row['to'] == each_v['from'] and row['to'] not in temp:
                        temp.append(each_v['from'])
                temp.append(row['to'])
                if temp:
                    clusters.append({'array':temp, 'color': KELLY_COLORS_HEX[random.randint(0,19)]})
            for node in self.data['nodes']:
                for c in clusters:
                    if node['label'] in c['array']:
                        node['color'] = c['color']
    ...
    def create(self, directed=False, vis_opts=None):
        ...
        """
        # create the app
        app = dash.Dash(__name__,server=tattva, url_base_pathname='/graph_data/', external_stylesheets=[dbc.themes.BOOTSTRAP])
    ...
    def plot(self, debug=False, host="127.0.0.1", port="8050", directed=False, vis_opts=None):
        ...
        # run the server
        return app
        # tattva = DispatcherMiddleware(tattva, {
        #     '/dash1': app.server})
...
def instantiate_dash_plot(relations):
    def _process_relations(relations):
        edges, nodes = [], []
        with open('relation.json','r') as fp:
    ...
    return DashPoweredNetworkGraph(edge_df, node_df).plot(directed=True)
dash_app1 = instantiate_dash_plot([])
 
@tattva.route('/graph_data')
def render_dashboard():
    return flask.redirect('/dash1')
 
if __name__ == "__main__":
    app = DispatcherMiddleware(tattva, {'/dash1': dash_app1.server})
    run_simple('0.0.0.0', 5000, app, use_reloader=True, use_debugger=True)
 
 
# if __name__ == '__main__':
#     tattva.run(debug=True)