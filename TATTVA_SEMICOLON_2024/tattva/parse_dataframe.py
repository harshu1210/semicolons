...
def parse_dataframe(edge_df, node_df=None):
    ...
    from jaal.layout import KELLY_COLORS_HEX
    for row in edge_df.to_dict(orient='records'):
        temp = [row['from']]
        for each_v in edge_df.to_dict(orient='records'):
    ...
    if node_df is None:
        ...
        node_image_url_flag = 'Cluster' in node_df.columns
        # create the node data
        for node in node_df.to_dict(orient='records'):
            color = '#97C2FC'
            for each in clusters:
                if row['from'] in each['array'] or row['to'] in each['array']:
                    color = each['color']
                    break
    ...
    for row in edge_df.to_dict(orient='records'):
        ...
        edges.append({**row, **{'id': row['from'] + "__" + row['to'],  'color': {'color': color}}})
